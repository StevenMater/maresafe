...
export function EditorPanel() {
  const { data, setField, addContact, removeContact, updateContact } = useFormData()
  const { t } = useTranslation()
  const [isFormCollapsed, setIsFormCollapsed] = useState(false)

  return (
    <div className="w-full flex flex-col items-center gap-2 pb-2" style={{ maxWidth: MAX_FORM_WIDTH }}>
      {/* Download action bar */}
      <div className="w-full bg-white border-[1.5px] border-[#d0dbe8] rounded-lg overflow-hidden">
        <DownloadBar />
      </div>
      ...
      >
        <ChevronDown
          size={14}
          className={cn("transition-transform duration-200", isFormCollapsed && "rotate-180")}
        />
        {t("form_toggle_label")}
        <ChevronDown
          size={14}
      ...
      >
        <div className="flex flex-col gap-2">
          <div className="bg-white border-[1.5px] border-[#d0dbe8] rounded-lg overflow-hidden">
