  ...
  function getCodeFeedback(): Feedback | null {
    switch (verifyState) {
      ...
      case "valid": {
        ...
        return { text: (depleted ? "✗ " : "✓ ") + text, className: depleted ? "text-red" : "text-green" }
      }
      case "invalid":
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 p-5 w-full sm:flex-1 sm:max-w-108 bg-white border border-[#d0dbe8] rounded-lg">
      ...
      <div className="flex flex-col items-center gap-0">
        <span className="text-base font-bold text-dark text-center">{t("activate_title")}</span>
        <span className="text-xs text-mid text-center">{t("activate_subtitle")}</span>
      </div>
